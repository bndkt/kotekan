	...
	return (tree: any) => {
		...
		visit(tree, "element", (node) => {
			if (node.tagName === "h2" && !node.properties.id) {
				node.properties.id = slugify(mToString(node));
			}
		});
	};
}

function rehypeAddMDXExports(getExports: (tree: any) => any) {
	return (tree: any) => {
...
type Child = {
	type: string;
	tagName: string;
	properties: { id: string; annotation: string };
	children: Child[];
};
...
function getSections(node: {
	...
	for (const child of node.children ?? []) {
		if (child.type === "element" && child.tagName === "h2") {
			sections.push(`{
				title: ${JSON.stringify(mToString(child))},
